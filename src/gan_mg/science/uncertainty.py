...
def _stable_logsumexp(x: np.ndarray) -> float:  # type: ignore[type-arg]
    m = float(np.max(x))
    return m + float(np.log(np.sum(np.exp(x - m))))


def compute_weights(dE: np.ndarray, T: float, kB: float, top_k: int = 5) -> tuple[np.ndarray, float, float, float]:  # type: ignore[type-arg]
    if dE.size == 0:
...
def bootstrap_gibbs_for_group(
    dE: np.ndarray,  # type: ignore[type-arg]
    Emin: float,
    T: float,
    kB: float,
    B: int,
