...
def analyze_from_csv(csv_path: Path, temperature_K: float) -> ThermoResult:
    ...
    return boltzmann_thermo_from_csv(csv_path, T=temperature_K)


def sweep_from_csv(csv_path: Path, temperatures_K: Sequence[float]) -> list[ThermoResult]:
...
def analyze_run(run_dir: Path, run_id: str, temperature_K: float) -> ThermoResult:
    ...
    return analyze_from_csv(csv_path=csv_path, temperature_K=temperature_K)


def sweep_run(run_dir: Path, run_id: str, temperatures_K: Sequence[float]) -> Path:
    ...
    rows = sweep_thermo_from_csv(csv_path, [float(value) for value in temperatures_K])
